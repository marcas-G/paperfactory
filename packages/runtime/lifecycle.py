...
class RuntimeSessionManager:
    ...
    def close_session(
        ...
        ts = self._now()
        updated = session.with_status(RuntimeSessionStatus.CLOSED, closed_by=actor, closed_at=ts)
    ...
    def cancel_session(
        ...
        self._check_no_busy_runs(session_id)
        ts = self._now()
        updated = session.with_status(RuntimeSessionStatus.CANCELLED, closed_by=actor, closed_at=ts)
        self._sessions.update(updated)
        self._emit(
    ...
    def _check_no_busy_runs(self, session_id: RuntimeSessionId) -> None:
        for run in self._runs.list_for_session(session_id):
            if run.status in RUN_NON_TERMINAL:
                raise SessionBusyError(f"session {session_id} has non-terminal run {run.run_id}")
...
class RuntimeRunManager:
    ...
    def create_run(
        ...
        *,
        ...
        if session.status is not RuntimeSessionStatus.OPEN:
            raise IllegalRunTransitionError(f"session {session_id} is not OPEN")
        ts = self._now()
        run = RuntimeRun(
    ...
    def pause_run(
        ...
        *,
        ...
        self._runs.update(updated)
        self._emit(
            session, RuntimeEventType.RUN_WAITING, updated, RunStatus.WAITING, ts, actor, prev
        )
        return updated
    ...
    def resume_run(
        ...
        self._runs.update(updated)
        self._emit(
            session, RuntimeEventType.RUN_RESUMED, updated, RunStatus.RUNNING, ts, actor, prev
        )
        return updated
    ...
    def fail_run(
        ...
        *,
        ...
        self._emit(
            session, RuntimeEventType.RUN_FAILED, updated_run, RunStatus.FAILED, ts, actor, prev
        )
        return updated_run, updated_attempt
    ...
    def cancel_run(
        ...
        if active is not None:
            ...
            self._emit(
                session,
                RuntimeEventType.ATTEMPT_CANCELLED,
                run,
    ...
    def timeout_run(
        ...
        self._emit(
            session,
            RuntimeEventType.ATTEMPT_TIMED_OUT,
            updated_run,
            AttemptStatus.TIMED_OUT,
            ts,
            actor,
        ...
        self._emit(
            session,
            RuntimeEventType.RUN_TIMED_OUT,
            updated_run,
            RunStatus.TIMED_OUT,
            ts,
            actor,
    ...
    def _require_active_attempt(self, run_id: RuntimeRunId) -> ExecutionAttempt:
        active = [
            a for a in self._attempts.list_for_run(run_id) if a.status is AttemptStatus.RUNNING
        ]
        if not active:
            raise RuntimeInvariantViolationError(f"run {run_id} has no active RUNNING attempt")
        if len(active) > 1:
            raise RuntimeInvariantViolationError(
