...
export function createHonoApp(
  ...
  // GET /api/projects/:id/papers — all citations as Paper objects
  app.get("/api/projects/:id/papers", async (c) => {
    const id = c.req.param("id");
    const citations = await Effect.runPromise(objectStore.list("Citation"));
  ...
  // GET /api/papers/:citationId — single citation detail
  app.get("/api/papers/:citationId", async (c) => {
  ...
  app.post("/api/papers/:citationId/download-pdf", async (c) => {
    const citationId = c.req.param("citationId");
    const opt = await Effect.runPromise(objectStore.get(citationId, "Citation"));
    if (opt.isNone()) {
  ...
  app.get("/api/projects/:id/phases/:phaseName/versions", async (c) => {
    const id = c.req.param("id");
    const phaseName = c.req.param("phaseName");
    const runs = await Effect.runPromise(objectStore.list("PhaseRun"));
  ...
  app.get("/api/projects/:id/phases/:phaseName/compare", async (c) => {
    const id = c.req.param("id");
    const phaseName = c.req.param("phaseName");
