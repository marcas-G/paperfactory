...
export const middleware = (...args) => {
  ...
  return (Effect.isEffect(middleware) ? Layer.effectDiscard(Effect.flatMap(middleware, add)) : Layer.effectDiscard(add(middleware))).pipe(Layer.provide(Middleware.layer));
};
/**
...
export const middlewareScoped = (...args) => {
  ...
  const add = withContext ? middlewareAdd : middlewareAddNoContext;
  const middleware = apiFirst ? args[1] : args[0];
  return Layer.scopedDiscard(Effect.flatMap(middleware, add)).pipe(Layer.provide(Middleware.layer));
};
/**
