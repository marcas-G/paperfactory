...
def _signals(quality=0.8, cost=0.7, latency=0.6, reliability=0.9, available=True, reasons=()):
...
def test_sel_002_required_capability_subset_matches():
    ...
    rec = eng.evaluate(
        ...
        agent_version="v1",
        policy=_policy(),
        requirement=_req(required_capabilities=frozenset({ModelCapability.STRUCTURED_OUTPUT})),
...
def test_sel_003_missing_capability_excluded():
    ...
    eng, _, _, _ = _engine_with(
        agent=make_agent(aid="A", refs=(REF_A1,)),
        profiles=(
            make_profile("P-A", "v1", capabilities=frozenset({ModelCapability.TEXT_GENERATION})),
        ),
    )
    rec = eng.evaluate(
        agent_id=AgentId("A"),
        agent_version="v1",
        policy=_policy(),
        requirement=_req(required_capabilities=frozenset({ModelCapability.VISION_INPUT})),
        signals={REF_A1: _signals()},
    )
...
def test_sel_004_required_parameter_support_matches():
    ...
    eng, _, _, _ = _engine_with(
        agent=make_agent(aid="A", refs=(REF_A1,)),
        profiles=(
            make_profile("P-A", "v1", supported_parameters=frozenset({ModelParameter.TEMPERATURE})),
        ),
    )
    rec = eng.evaluate(
        agent_id=AgentId("A"),
        agent_version="v1",
        policy=_policy(),
        requirement=_req(required_parameters=frozenset({ModelParameter.TEMPERATURE})),
        signals={REF_A1: _signals()},
    )
    assert rec.status is ModelSelectionStatus.RECOMMENDED
...
def test_sel_005_missing_parameter_support_excluded():
    ...
    rec = eng.evaluate(
        ...
        agent_version="v1",
        policy=_policy(),
        requirement=_req(required_parameters=frozenset({ModelParameter.SEED})),
        signals={REF_A1: _signals()},
    )
...
def test_sel_006_allowed_provider_matches():
    ...
    eng, _, _, _ = _engine_with(
        agent=make_agent(aid="A", refs=(REF_A1,)), profiles=(make_profile("P-A", "v1"),)
    )
    rec = eng.evaluate(
        agent_id=AgentId("A"),
        agent_version="v1",
        policy=_policy(),
        requirement=_req(allowed_providers=frozenset({"fake"})),
        signals={REF_A1: _signals()},
    )
    assert rec.status is ModelSelectionStatus.RECOMMENDED
...
def test_sel_007_provider_not_allowed_excluded():
    ...
    rec = eng.evaluate(
        ...
        agent_version="v1",
        policy=_policy(),
        requirement=_req(allowed_providers=frozenset({"openai"})),
        signals={REF_A1: _signals()},
    )
...
@pytest.mark.parametrize(
    "q,c,lat,rel",
    [(-0.1, 0.5, 0.5, 0.5), (0.5, -0.1, 0.5, 0.5), (0.5, 0.5, -0.1, 0.5), (0.5, 0.5, 0.5, -0.1)],
)
...
@pytest.mark.parametrize(
    "q,c,lat,rel",
    [(1.1, 0.5, 0.5, 0.5), (0.5, 1.1, 0.5, 0.5), (0.5, 0.5, 1.1, 0.5), (0.5, 0.5, 0.5, 1.1)],
)
...
def test_sel_025_quality_higher_ranks_higher():
    ...
    policy = ModelSelectionPolicy(
        policy_id=ModelSelectionPolicyId("p"),
        version="v1",
...
def test_sel_029_components_sum_equals_total():
    ...
    s = (
        comp.quality_component
        + comp.cost_efficiency_component
        + comp.latency_component
        + comp.reliability_component
    )
...
def test_sel_030_unavailable_excluded_before_scoring():
    ...
    rec = eng.evaluate(
        ...
        requirement=_req(),
        signals={
            REF_A1: _signals(quality=0.99),
            REF_B1: _signals(available=False, reasons=("DOWN",)),
        },
    )
    assert rec.ranked_candidates[0].profile_ref == REF_A1
    assert any(
        e.reason is ModelSelectionExclusionReason.UNAVAILABLE for e in rec.excluded_candidates
    )
...
def test_sel_031_same_input_same_ranking():
    ...
    r1 = eng1.evaluate(
        agent_id=AgentId("A"), agent_version="v1", policy=_policy(), requirement=_req(), signals=sig
    )
    r2 = eng2.evaluate(
        agent_id=AgentId("A"), agent_version="v1", policy=_policy(), requirement=_req(), signals=sig
    )
    ...
    assert [c.score for c in r1.ranked_candidates] == [c.score for c in r2.ranked_candidates]
...
def test_missing_allowed_profile_is_configuration_error():
    ...
    eng, _, _, _ = _engine_with(
        agent=make_agent(aid="A", refs=(REF_A1,)), profiles=()
    )  # P-A/v1 NOT registered
