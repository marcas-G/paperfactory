...
def _selection_setup():
    ...
    for key, prov in (("fast", FAST), ("quality", QUALITY), ("disabled", FAKE)):
        profile_reg.register(ModelExecutionProfile(
            profile_id=refs[key].profile_id, version="v1",
            name=key, description="d", provider=prov, model=MODEL,
...
def test_m3_sel_001_deterministic_profile_selection():
    ...
    rec1 = eng.evaluate(agent_id=AgentId("A"), agent_version="v1",
                        policy=policy, requirement=requirement, signals=signals)
    # disabled excluded
    ...
    rec2 = eng.evaluate(agent_id=AgentId("A"), agent_version="v1",
                        policy=policy, requirement=requirement, signals=signals)
    assert [c.profile_ref for c in rec1.ranked_candidates] == \
           [c.profile_ref for c in rec2.ranked_candidates]
    assert [c.score for c in rec1.ranked_candidates] == \
           [c.score for c in rec2.ranked_candidates]
...
def test_m3_sel_002_recommendation_is_not_binding():
    ...
    sm = RuntimeSessionManager(sess_store, run_store, sink,
        session_id_factory=Counter("s"), event_id_factory=Counter("e"), now=lambda: TZ)
    rm = RuntimeRunManager(sess_store, run_store, att_store, sink,
        run_id_factory=Counter("r"), attempt_id_factory=Counter("a"),
        event_id_factory=Counter("e"), now=lambda: TZ)
    ...
    mgr = AgentBindingManager(sess_store, run_store, agent_reg, profile_reg,
        ...
        binding_id_factory=Counter("b"), event_id_factory=Counter("e"), now=lambda: TZ)
